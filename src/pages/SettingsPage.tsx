        ...
        <>
            ...
            <div className="page-content fade-in" style={{ maxWidth: 900 }}>
                ...
                {activeTab === 'general' && (
                    <>
                        {/* Role & Access */}
                        <div className="card" style={{ marginBottom: 24, borderLeft: `4px solid ${isAdmin ? 'var(--accent)' : isUserPlus ? 'var(--green)' : 'var(--yellow)'}` }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 16 }}>
                                {isAdmin ? <Shield size={20} style={{ color: 'var(--accent)' }} aria-hidden="true" /> : isUserPlus ? <User size={20} style={{ color: 'var(--green)' }} aria-hidden="true" /> : <GraduationCap size={20} style={{ color: 'var(--yellow)' }} aria-hidden="true" />}
                            ...
                            {!settings.adminPin && (
                                <p className="text-muted text-xs" style={{ marginTop: 12 }}>
                                    <AlertCircle size={11} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 4 }} aria-hidden="true" />
                                    No admin password is set — anyone can switch to Administrator. Set a password in the{' '}
                                    <button className="btn btn-ghost btn-sm" style={{ padding: '0 4px', fontSize: 'inherit' }}
                                        onClick={() => { if (isAdmin) setActiveTab('administration'); else requestRoleSwitch('admin'); }}>
                                        Administration tab
                                    </button>
                                    {' '}to restrict access.
                                </p>
                        ...
                        <div className="card" style={{ marginBottom: 24 }}>
                            ...
                            <div className="grid-2" style={{ gap: 16 }}>
                                <div className="form-group">
                                    ...
                                    <select id="setting-theme" value={settings.theme}
                                ...
                                {isUserPlus && (
                                    <div className="form-group">
                                        ...
                                        {accentError && <div id="accent-error" className="text-xs" style={{ color: 'var(--red)', marginTop: 4 }}>{t('settings.accent_color_invalid')}</div>}
                                    </div>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="card" style={{ marginBottom: 24, borderLeft: '4px solid var(--accent)' }}>
                            ...
                            <div className="form-group">
                                ...
                                <select id="setting-language" value={settings.language}
                        ...
                        <div className="card">
                            ...
                            <button
                                ...
                                onClick={() => { updateSettings({ hasSeenTutorial: false }); navigate('/'); }}
                            >
                        ...
                        <div className="card" style={{ marginBottom: 24 }}>
                            ...
                            <div className="grid-2" style={{ gap: 16 }}>
                                <div className="form-group">
                                    ...
                                    <select id="setting-grade-scale" value={settings.defaultGradeScaleId}
                                ...
                                <div className="form-group" style={{ gridColumn: '1 / -1' }}>
                                    <label htmlFor="setting-matchup-limit" style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                        ...
                                        <div key={tmpl.id} style={{ display: 'flex', alignItems: 'center', gap: 12, background: 'var(--bg-elevated)', borderRadius: 10, padding: '12px 16px', border: settings.exportTemplateId === tmpl.id ? '1.5px solid var(--accent)' : '1px solid var(--border)' }}>
                                            ...
                                            <div style={{ flex: 1 }}>
                                                ...
                                                <div style={{ fontSize: '0.78rem', color: 'var(--text-muted)', marginTop: 2 }}>
                                                    {tmpl.levelHeaders.length > 0 ? tmpl.levelHeaders.join(' · ') : t('settings.no_level_headers')}
                                                </div>
                                            </div>
                            ...
                            <div className="card">
                                <div className="admin-locked-card">
                                    ...
                                    <p className="text-muted text-sm" style={{ maxWidth: 400 }}>
                                        These settings control database connections, API integrations, and system-level configuration.
                                        They are restricted to the Administrator role.
                                    </p>
                                    {settings.adminPin ? (
                                            ...
                                            <div className="form-group" style={{ marginBottom: 0 }}>
                                                ...
                                                <div style={{ display: 'flex', gap: 8 }}>
                                                    <input
                                                        ...
                                                        onKeyDown={e => { if (e.key === 'Enter') { setPendingRole('admin'); confirmPinAndSwitch(); } }}
                                                ...
                                                {pinError && <div className="text-xs" style={{ color: 'var(--red)', marginTop: 4 }}>Incorrect password</div>}
                                            </div>
                                            <button className="btn btn-primary"
                                                onClick={() => { setPendingRole('admin'); confirmPinAndSwitch(); }}>
                                                <Shield size={15} aria-hidden="true" /> Switch to Administrator
                                            </button>
                                        </div>
                                    ) : (
                                        <button className="btn btn-primary"
                                            onClick={() => applyRoleSwitch('admin')}>
                                            <Shield size={15} aria-hidden="true" /> Switch to Administrator
                                        </button>
                                    )}
                                </div>
                            ...
                            <>
                                ...
                                <div className="card" style={{ marginBottom: 24, borderLeft: '4px solid var(--accent)' }}>
                                    ...
                                    <div className="form-group">
                                        ...
                                        <input id="setting-standards-key" type="password"
                                    ...
                                    <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 16 }}>
                                        ...
                                        <span style={{ marginLeft: 'auto', display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.8rem' }}>
                                            {dbStatus.isConnected
                                                ? <><Wifi size={14} style={{ color: 'var(--green)' }} aria-hidden="true" /><span style={{ color: 'var(--green)' }}>Connected</span></>
                                                : <><WifiOff size={14} style={{ color: 'var(--text-muted)' }} aria-hidden="true" /><span style={{ color: 'var(--text-muted)' }}>Not connected</span></>
                                            }
                                        </span>
                                    </div>
                                    ...
                                    {dbStatus.isConnected ? (
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                                                    ...
                                                    {!otpSent ? (
                                                        <div style={{ display: 'flex', gap: 8 }}>
                                                            <input type="email" placeholder="your@email.com" value={dbEmail}
                                                                onChange={e => setDbEmail(e.target.value)} style={{ flex: 1 }} />
                                                        ...
                                                        <div style={{ display: 'flex', gap: 8 }}>
                                                            <input type="text" placeholder="Enter 6-digit code" value={dbOtp}
                                                                onChange={e => setDbOtp(e.target.value)} maxLength={6} style={{ flex: 1 }} />
                                                            <button className="btn btn-primary btn-sm"
                                                                onClick={async () => {
                                            ...
                                            <div style={{ borderTop: '1px solid var(--border)', paddingTop: 12 }}>
                                                <button className="btn btn-ghost btn-sm" style={{ color: 'var(--red)', fontSize: '0.8rem' }}
                                                    onClick={async () => {
                                ...
                                <div className="card" style={{ marginBottom: 24 }}>
                                    ...
                                    <div style={{ marginTop: 10, padding: '8px 12px', background: 'color-mix(in srgb, var(--yellow) 10%, transparent)', border: '1px solid color-mix(in srgb, var(--yellow) 40%, transparent)', borderRadius: 8, fontSize: '0.8rem', color: 'var(--text-muted)' }}>
                                        <AlertCircle size={12} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 6 }} aria-hidden="true" />
                                        Importing a backup will <strong>overwrite all current data</strong>. This action cannot be undone.
                                    </div>
                                </div>
                                ...
                                <div className="card" style={{ marginBottom: 24, borderLeft: '4px solid var(--accent)' }}>
                                    <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 16 }}>
                                        ...
                                        <div>
                                            ...
                                            <p className="text-muted text-xs" style={{ marginTop: 2 }}>
                                                {settings.adminPin ? 'A password is set. Required to switch back to Administrator role.' : 'No password set — anyone can switch to Administrator.'}
                                            </p>
                                        </div>
                                        {settings.adminPin && (
                                            <span className="badge badge-green" style={{ marginLeft: 'auto' }}>Protected</span>
                                        )}
                                    </div>
                                    ...
                                    {pinSetupMode === 'idle' ? (
                                        <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                                            {!settings.adminPin ? (
                                                <button className="btn btn-primary btn-sm" onClick={() => { setPinSetupMode('setting'); setPinSetupError(''); }}>
                                                    <Lock size={14} aria-hidden="true" /> Set admin password
                                                </button>
                                            ) : (
                                                <>
                                                    <button className="btn btn-secondary btn-sm" onClick={() => { setPinSetupMode('changing'); setPinSetupError(''); }}>
                                                        <Save size={14} aria-hidden="true" /> Change password
                                                    </button>
                                                    <button className="btn btn-ghost btn-sm" style={{ color: 'var(--red)' }}
                                                        onClick={() => { setPinSetupMode('removing'); setPinSetupError(''); }}>
                                        ...
                                        <div style={{ display: 'flex', flexDirection: 'column', gap: 10, maxWidth: 340 }}>
                                            <h4 style={{ margin: 0, fontSize: '0.9rem' }}>
                                                {pinSetupMode === 'setting' ? 'Set admin password' : pinSetupMode === 'changing' ? 'Change admin password' : 'Remove admin password'}
                                            </h4>

                                            {(pinSetupMode === 'changing' || pinSetupMode === 'removing') && (
                                                <div className="form-group" style={{ marginBottom: 0 }}>
                                                    <label htmlFor="admin-current-pin">Current password</label>
                                                    <input id="admin-current-pin" type="password"
                                                        value={currentPinVerify}
                                                        onChange={e => { setCurrentPinVerify(e.target.value); setPinSetupError(''); }}
                                            ...
                                            {pinSetupMode !== 'removing' && (
                                                <>
                                                    <div className="form-group" style={{ marginBottom: 0 }}>
                                                        ...
                                                        <div style={{ display: 'flex', gap: 8 }}>
                                                            <input id="admin-new-pin" type={showNewPin ? 'text' : 'password'}
                                                                value={newPin}
                                                                onChange={e => { setNewPin(e.target.value); setPinSetupError(''); }}
                                                    ...
                                                    <div className="form-group" style={{ marginBottom: 0 }}>
                                                        ...
                                                        <input id="admin-confirm-pin" type="password"
                                                            value={confirmPin}
                                                            onChange={e => { setConfirmPin(e.target.value); setPinSetupError(''); }}
                                                            autoComplete="new-password"
                                                        />
                                                    </div>
                                                </>
                                            ...
                                            {pinSetupError && (
                                                <div className="text-xs" style={{ color: 'var(--red)' }}>{pinSetupError}</div>
                                            )}

                                            <p className="text-muted text-xs">
                                                <AlertCircle size={11} style={{ display: 'inline', verticalAlign: 'middle', marginRight: 4 }} aria-hidden="true" />
                                                This is UI access control for normal use, not cryptographic security. The password is stored locally in your browser.
                                            </p>

                                            <div style={{ display: 'flex', gap: 8 }}>
                                                <button className="btn btn-ghost btn-sm" onClick={resetPinSetup}>Cancel</button>
                                                {pinSetupMode === 'removing' ? (
                                                    <button className="btn btn-danger btn-sm" onClick={handleRemovePin}>Remove password</button>
                                                ) : (
                                                    <button className="btn btn-primary btn-sm" onClick={handleSaveNewPin}>Save password</button>
                                                )}
                                            </div>
                                        </div>
                                    )}
                                ...
                                <div className="card">
                                    ...
                                    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                                        {(['admin', 'user', 'student'] as UserRole[]).map(r => (
                                            <div key={r} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '12px 14px', background: 'var(--bg-elevated)', borderRadius: 10, border: role === r ? '1.5px solid var(--accent)' : '1px solid var(--border)' }}>
                                                <span className={`role-badge ${ROLE_META[r].badgeClass}`} style={{ flexShrink: 0, marginTop: 1 }}>
                                                    {ROLE_META[r].icon} {ROLE_META[r].label}
                                                </span>
                                                <p className="text-muted text-sm" style={{ margin: 0 }}>{ROLE_META[r].description}</p>
                                                {role === r && <span className="badge badge-blue" style={{ marginLeft: 'auto', flexShrink: 0 }}>Current</span>}
                                            </div>
                                        ))}
                                    </div>
                                </div>
            ...
            {deleteScaleId && (
                <Modal titleId="delete-scale-title" onClose={() => setDeleteScaleId(null)} maxWidth={420}>
                    <div className="modal-header">
                        <h3 id="delete-scale-title" style={{ margin: 0 }}>{t('settings.delete_scale_title')}</h3>
                        <button className="btn btn-ghost btn-icon btn-sm" onClick={() => setDeleteScaleId(null)} aria-label="Close">✕</button>
                    </div>
            ...
            {showPinDialog && (
                <Modal titleId="pin-dialog-title" onClose={() => { setShowPinDialog(false); setPinInput(''); setPinError(false); }} maxWidth={380}>
                    <div className="modal-header">
                        <h3 id="pin-dialog-title" style={{ margin: 0, display: 'flex', alignItems: 'center', gap: 8 }}>
                            <Shield size={16} style={{ color: 'var(--accent)' }} aria-hidden="true" /> Switch to Administrator
                        </h3>
                        <button className="btn btn-ghost btn-icon btn-sm" onClick={() => { setShowPinDialog(false); setPinInput(''); setPinError(false); }} aria-label="Close">✕</button>
                    </div>
                        ...
                        <div className="form-group" style={{ marginBottom: 0 }}>
                            ...
                            <div style={{ display: 'flex', gap: 8 }}>
                                <input
                                    ...
                                    value={pinInput}
                                    onChange={e => { setPinInput(e.target.value); setPinError(false); }}
                                    onKeyDown={e => { if (e.key === 'Enter') confirmPinAndSwitch(); }}
                                    aria-invalid={pinError ? 'true' : undefined}
                            ...
                            {pinError && <div id="pin-error" className="text-xs" style={{ color: 'var(--red)', marginTop: 4 }}>Incorrect password. Please try again.</div>}
                        </div>
                        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
                            <button className="btn btn-ghost" onClick={() => { setShowPinDialog(false); setPinInput(''); setPinError(false); }}>Cancel</button>
