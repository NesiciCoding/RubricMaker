...
export default function StatisticsPage() {
        ...
        return rubrics
            ...
            .map(r => {
                ...
                const median = sorted.length % 2 === 0
                    ...
                    : sorted[mid];
                return { rubricName: r.name, date: r.createdAt, avg: parseFloat(avg.toFixed(1)), median: parseFloat(median.toFixed(1)) };
            })
    ...
    const tableData = useMemo(() => {
        ...
        return data.sort((a, b) => {
            ...
            if (sortKey === 'name') { valA = a.student.name.toLowerCase(); valB = b.student.name.toLowerCase(); }
            else if (sortKey === 'score') { valA = a.summary.modifiedPercentage; valB = b.summary.modifiedPercentage; }
            else if (sortKey === 'raw') { valA = a.summary.rawScore; valB = b.summary.rawScore; }
            else if (sortKey === 'grade') { valA = a.summary.letterGrade; valB = b.summary.letterGrade; }
            else { valA = a.summary.gradedCount / (a.summary.totalCriteria || 1); valB = b.summary.gradedCount / (b.summary.totalCriteria || 1); }
    ...
    const studentViewTableData = useMemo(() => {
        ...
        return studentRubrics
            ...
            .map(sr => {
                ...
                const gScale = gScaleId === 'none' ? null : (gradeScales.find(g => g.id === gScaleId) ?? gradeScales[0]);
    ...
    const studentRadarData = useMemo((): CriterionRadarDataPoint[] | null => {
        ...
        r.criteria.forEach(c => {
            ...
            const max = Math.max(...c.levels.map(l => l.maxPoints), 1);
            classAvgMap[c.id] = scores.length > 0
                                ...
                                <div className="form-group" style={{ flex: 1, maxWidth: 320, marginBottom: 0 }}>
                                    ...
                                    <select value={selectedStudentRubricId} onChange={e => setSelectedStudentRubricId(e.target.value)}>
                                        ...
                                        {studentRubrics
                                            .filter(sr => sr.studentId === selectedStudentId)
                                            .map(sr => {
                                                const r = sr.rubricSnapshot || rubrics.find(r => r.id === sr.rubricId);
                                                return r ? <option key={sr.id} value={sr.id}>{r.name}</option> : null;
                                            })}
                                    </select>
                                </div>
                            )}
                                            ...
                                            {tableData.map(({ sr, student, summary }) => (
                                                <tr key={sr.id} style={{ borderLeft: `4px solid ${summary.gradeColor}`, background: `${summary.gradeColor}08` }}>
                                                    <td style={{ fontWeight: 500, paddingLeft: 16 }}>
                                                        {student.name}
                                                        {sr.notHandedIn && <span className="text-muted text-sm" style={{ marginLeft: 6 }}>(NHI)</span>}
                                                    </td>
                                                    <td>{summary.modifiedPercentage.toFixed(1)}%</td>
                                                    <td>{summary.rawScore}/{summary.maxRawScore}</td>
                                                    <td><span style={{ color: summary.gradeColor, fontWeight: 700 }}>{summary.letterGrade}</span></td>
                                            ...
                                            <div className="card" style={{ marginBottom: 16, padding: '12px 16px' }}>
                                                ...
                                                <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8 }}>
                                                    {rubricPeers.map((peer, i) => {
                                                        ...
                                                        const maxReached = !isSelected && comparedStudentIds.length >= 3;
                                                        return (
                                                            <button
                                                                key={peer.id}
                                                                ...
                                                                disabled={maxReached}
                                                                onClick={() => setComparedStudentIds(prev =>
