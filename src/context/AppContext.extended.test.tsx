...
describe('AppContext — extended actions', () => {
    ...
    it('updates a student', () => {
        const { result } = renderHook(() => useApp(), { wrapper });
        act(() => { result.current.addStudent({ name: 'Bob', classId: 'c1' }); });
        act(() => { result.current.updateStudent({ ...result.current.students[0], name: 'Bobby' }); });
        expect(result.current.students[0].name).toBe('Bobby');
    });

    it('adds, updates, and deletes a class', () => {
        const { result } = renderHook(() => useApp(), { wrapper });
        act(() => { result.current.addClass({ name: 'Class A' }); });
        const cls = result.current.classes[0];
        act(() => { result.current.updateClass({ ...cls, name: 'Class B' }); });
        expect(result.current.classes[0].name).toBe('Class B');
        act(() => { result.current.deleteClass(cls.id); });
    ...
    it('deleteClass with deleteStudents=true removes students in that class', () => {
        ...
        act(() => { result.current.addClass({ name: 'Class A' }); });
        const cls = result.current.classes[0];
        act(() => { result.current.addStudent({ name: 'Alice', classId: cls.id }); });
        expect(result.current.students).toHaveLength(1);
        act(() => { result.current.deleteClass(cls.id, true); });
        expect(result.current.students).toHaveLength(0);
    });
    ...
    it('saves and deletes a self assessment', () => {
        ...
        act(() => { result.current.saveSelfAssessment(sa); });
        expect(result.current.selfAssessments[0].id).toBe('sa1');
        act(() => { result.current.deleteSelfAssessment('sa1'); });
    ...
    it('saves and updates a speaking session', () => {
        const { result } = renderHook(() => useApp(), { wrapper });
        const session = { id: 'ss1', rubricId: 'r1', studentId: 's1', criteria: [], overallComment: '', gradedAt: '2024-01-01' } as any;
        act(() => { result.current.saveSpeakingSession(session); });
        expect(result.current.speakingSessions[0].id).toBe('ss1');
        act(() => { result.current.saveSpeakingSession({ ...session, overallComment: 'Updated' }); });
    ...
    it('deletes a speaking session', () => {
        ...
        const session = { id: 'ss1', rubricId: 'r1', studentId: 's1', criteria: [], overallComment: '', gradedAt: '2024-01-01' } as any;
        act(() => { result.current.saveSpeakingSession(session); });
        act(() => { result.current.deleteSpeakingSession('ss1'); });
    ...
    it('saves a rubric version', () => {
        const { result } = renderHook(() => useApp(), { wrapper });
        act(() => { result.current.addRubric(makeRubric()); });
        const rubricId = result.current.rubrics[0].id;
        act(() => { result.current.saveRubricVersion(rubricId, 'v1'); });
    ...
    it('restores a rubric version', () => {
        ...
        const rubricId = result.current.rubrics[0].id;
        act(() => { result.current.saveRubricVersion(rubricId, 'v1'); });
        act(() => { result.current.updateRubric({ ...result.current.rubrics[0], name: 'Modified' }); });
        act(() => { result.current.restoreRubricVersion(rubricId, 0); });
        expect(result.current.rubrics[0].name).toBe('R1');
    });

    it('saveRubricVersion does nothing for unknown rubricId', () => {
        const { result } = renderHook(() => useApp(), { wrapper });
        act(() => { result.current.saveRubricVersion('unknown'); });
        expect(result.current.rubrics).toHaveLength(0);
    });

    it('restoreRubricVersion does nothing for unknown rubricId', () => {
        const { result } = renderHook(() => useApp(), { wrapper });
        act(() => { result.current.restoreRubricVersion('unknown', 0); });
        expect(result.current.rubrics).toHaveLength(0);
    });
    ...
    it('restoreRubricVersion does nothing for out-of-bounds index', () => {
        ...
        act(() => { result.current.addRubric(makeRubric()); });
        const rubricId = result.current.rubrics[0].id;
        act(() => { result.current.restoreRubricVersion(rubricId, 99); });
    ...
    it('syncRubricSnapshot updates student rubrics with new criteria', () => {
        ...
        act(() => { result.current.addStudent({ name: 'Alice', classId: 'c1' }); });
        act(() => { result.current.createStudentRubric(rubric.id, result.current.students[0].id); });
        ...
        act(() => { result.current.syncRubricSnapshot(rubric.id, updatedRubric); });
    ...
    it('adds, updates, and deletes vocabulary items', () => {
        ...
        act(() => { result.current.deleteVocabularyItem(rubricId, itemId!); });
    ...
    it('saves and deletes analysis results', () => {
        ...
        const ar = {
            ...
            detectedItems: [], grammarErrors: [], grammarCheckerUsed: 'none' as const,
        };
        act(() => { result.current.saveAnalysisResult(ar); });
        expect(result.current.analysisResults[0].id).toBe('ar1');

        // Save again (update path)
        act(() => { result.current.saveAnalysisResult({ ...ar, extractedText: 'updated' }); });
        expect(result.current.analysisResults).toHaveLength(1);
        expect(result.current.analysisResults[0].extractedText).toBe('updated');

        act(() => { result.current.deleteAnalysisResult('ar1'); });
