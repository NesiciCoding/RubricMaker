...
import {
    fetchJurisdictions,
    fetchStandardSets,
    fetchStandardSetDetail,
    flattenStandards,
    type CspJurisdiction,
    type CspStandardSet,
    type CspStandard,
...
export default function StandardsPickerModal({ apiKey, onSelect, onClose }: Props) {
    ...
    const displayStandards = useMemo(() => {
        ...
        if (sortOrder === 'alpha-asc') {
            ...
            list.sort((a, b) =>
                (b.statementNotation || b.description).localeCompare(a.statementNotation || a.description)
            );
        }
        return list;
    ...
    return (
        <Modal
            titleId="standards-picker-title"
            onClose={onClose}
            maxWidth={800}
                        ...
                        <React.Fragment key={s.key}>
                            ...
                            <button
                                ...
                                style={{
                                    padding: '2px 6px',
                                    fontSize: '0.8rem',
                                    opacity: i < steps.findIndex((x) => x.key === step) ? 1 : 0.5,
                                }}
                                    ...
                                    >
                                        <div>
                                            ...
                                            <div className="text-xs text-muted" style={{ marginTop: 4 }}>
                                                {s.subject}
                                                {s.educationLevels?.length ? ` · ${s.educationLevels.join(', ')}` : ''}
                                            </div>
                                        </div>
                                        ...
                                        >
                                            <button
                                                ...
                                                style={{
                                                    flexShrink: 0,
                                                    marginTop: 2,
                                                    color: isFav ? 'var(--yellow)' : 'var(--text-dim)',
                                                }}
                                            >
                                            ...
                                            >
                                                {std.statementNotation && (
                                                    <span
                                                        className="badge badge-blue"
                                                        style={{ flexShrink: 0, marginTop: 1, fontSize: '0.75rem' }}
                                                    >
                                                        {std.statementNotation}
                                                    </span>
                                                )}
