...
export default function EssayImportModal({
    rubricId, studentId, studentName, teacherKey,
    onImport, onClose,
    onFetchSubmissions, onGetSignedUrl, onDeleteSubmission,
}: Props) {
    ...
    const handleImportCode = useCallback(() => {
        ...
        onImport({ name: filename, mimeType: 'text/html', dataUrl, rubricId, studentId, size: submission.contentHtml.length });
    ...
    const handleImportFromDb = useCallback(async (sub: DbSubmission) => {
        if (!onGetSignedUrl) return;
        setImportingId(sub.id); setDbError('');
        try {
            const url = await onGetSignedUrl(sub.storagePath);
            if (!url) { setDbError('Could not get download URL. Try again.'); setImportingId(null); return; }
    ...
    const handleDelete = useCallback(async (sub: DbSubmission) => {
        if (!onDeleteSubmission) return;
        if (!confirm(`Delete this submission from ${sub.studentEmail ?? 'student'}? This cannot be undone.`)) return;
                ...
                <div style={{ display: 'flex', borderBottom: '1px solid var(--border)', background: 'var(--bg-elevated)' }}>
                    {(['database', 'code'] as Tab[]).map(t => (
                        <button key={t} onClick={() => setTab(t)}
                            style={{ flex: 1, padding: '10px 0', border: 'none', background: 'none', fontWeight: tab === t ? 700 : 400, color: tab === t ? 'var(--accent)' : 'var(--text-muted)', borderBottom: tab === t ? '2px solid var(--accent)' : '2px solid transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, fontSize: '0.875rem' }}>
                            {t === 'database' ? <><Database size={13} /> From database</> : <><FileText size={13} /> Paste code</>}
                        </button>
                    ))}
                </div>
            )}
                ...
                {tab === 'database' && hasDb && (
                    <>
                        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                            ...
                            <button className="btn btn-ghost btn-icon btn-sm" title="Refresh" onClick={loadSubmissions} disabled={loadingDb}>
                                <RefreshCw size={13} style={{ animation: loadingDb ? 'spin 1s linear infinite' : undefined }} />
                            </button>
                        </div>
                        ...
                        {loadingDb ? (
                            <p className="text-muted text-sm" style={{ textAlign: 'center', padding: '16px 0' }}>Loading…</p>
                            ...
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                                {submissions.map(sub => (
                                    <div key={sub.id} style={{ display: 'flex', alignItems: 'center', gap: 10, background: 'var(--bg-elevated)', borderRadius: 8, padding: '10px 12px' }}>
                    ...
                    <>
                        ...
                        <div className="form-group" style={{ marginBottom: 0 }}>
                            ...
                            <textarea value={code} onChange={e => { setCode(e.target.value); setError(''); }}
                                rows={5} placeholder="Paste the student's submission code here…"
                        ...
                        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 8 }}>
                            <button className="btn btn-secondary btn-sm" onClick={onClose}>Cancel</button>
                            <button className="btn btn-primary btn-sm" onClick={handleImportCode} disabled={!code.trim()}>
