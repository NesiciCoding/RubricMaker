...
import { X, Upload, CheckCircle, Database, RefreshCw, FileText, Trash2, AlertTriangle, TrendingDown } from 'lucide-react';
...
export default function EssayImportModal({
    ...
    const [meta, setMeta] = useState<{ wordCount: number; submittedAt: string; wordLimitStatus?: 'ok' | 'under' | 'over' | null } | null>(null);
    ...
    const handleImportCode = useCallback(() => {
        ...
        setMeta({ wordCount: submission.wordCount, submittedAt: submission.submittedAt, wordLimitStatus: submission.wordLimitStatus });
        setImported(true);
    ...
    const handleImportFromDb = useCallback(
        async (sub: DbSubmission) => {
            ...
            try {
                ...
                setMeta({ wordCount: sub.wordCount, submittedAt: sub.submittedAt, wordLimitStatus: sub.wordLimitStatus });
                setImported(true);
            } catch {
            ...
            <Modal titleId="essay-import-title" onClose={onClose} maxWidth={500}>
                ...
                >
                    ...
                    {meta.wordLimitStatus === 'over' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.8rem', fontWeight: 600, color: '#dc2626', background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 6, padding: '4px 10px' }}>
                            <AlertTriangle size={13} /> Over word limit
                        </div>
                    )}
                    {meta.wordLimitStatus === 'under' && (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: '0.8rem', fontWeight: 600, color: '#d97706', background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 6, padding: '4px 10px' }}>
                                ...
                                {submissions.map((sub) => (
                                    ...
                                    >
                                        <div style={{ flex: 1, minWidth: 0 }}>
                                            ...
                                            <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', fontSize: '0.75rem', color: 'var(--text-muted)' }}>
                                                <span>{sub.wordCount} words · {new Date(sub.submittedAt).toLocaleString()}</span>
                                                {sub.wordLimitStatus === 'over' && (
                                                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontWeight: 600, color: '#dc2626', background: '#fef2f2', border: '1px solid #fca5a5', borderRadius: 4, padding: '1px 6px' }}>
                                                        <AlertTriangle size={10} /> Over limit
                                                    </span>
                                                )}
                                                {sub.wordLimitStatus === 'under' && (
                                                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3, fontWeight: 600, color: '#d97706', background: '#fffbeb', border: '1px solid #fcd34d', borderRadius: 4, padding: '1px 6px' }}>
