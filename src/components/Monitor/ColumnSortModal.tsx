...
export default function ColumnSortModal({ rules, onApply, onClose }: ColumnSortModalProps) {
    ...
    return (
        <Modal
            titleId="column-sort-title"
            onClose={onClose}
            maxWidth={480}
            style={{ maxHeight: '80vh', overflowY: 'auto' }}
        >
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
                <h3 id="column-sort-title" style={{ margin: 0 }}>
                    {t('tests.monitor.grid.sort_title')}
                </h3>
                ...
                {draft.map((rule, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        ...
                        <select
                            ...
                            onChange={(e) =>
                                setDraft((prev) =>
                                    prev.map((r, j) => (j === i ? { ...r, key: e.target.value as ColumnSortKey } : r))
                                )
                            }
                            style={{ flex: 1 }}
                        >
                        ...
                        <button
                            ...
                            onClick={() =>
                                setDraft((prev) =>
                                    prev.map((r, j) => (j === i ? { ...r, dir: r.dir === 'asc' ? 'desc' : 'asc' } : r))
                                )
                            }
                        >
