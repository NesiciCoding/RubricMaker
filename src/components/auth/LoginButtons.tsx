...
export default function LoginButtons({ onEmailSuccess, supabaseReady, onNeedConfig }: LoginButtonsProps) {
    ...
    async function handleOAuth(provider: 'google' | 'ms-personal' | 'azure-ad') {
        if (!supabaseReady) { onNeedConfig?.(); return; }
        setError('');
        setBusy(provider);
        ...
        if (result.error) { setError(result.error); setBusy(null); }
    }
    ...
    async function handleVerifyOtp() {
        ...
        setBusy(null);
        if (result.error) setError(result.error);
        else { setDone(true); onEmailSuccess?.(); }
    }
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {/* Google */}
            {show('google') && (
                <button style={btnBase} disabled={!!busy}
            ...
            {/* Azure AD */}
            {show('azure_ad') && (
                <button style={{ ...btnBase, borderColor: '#bfdbfe', background: '#eff6ff' }}
                    disabled={!!busy}
                    ...
                    {busy === 'azure-ad'
                        ? <Loader2 size={18} style={{ animation: 'spin 1s linear infinite', flexShrink: 0 }} />
                        : <MicrosoftIcon />}
                    <span>Sign in with Microsoft <span style={{ fontSize: '0.78rem', fontWeight: 400, color: '#3b82f6' }}>(school / work)</span></span>
                </button>
            )}

            {/* Email OTP toggle */}
            {show('email') && (
                <button style={{ ...btnBase, background: 'transparent', border: 'none', padding: '6px 4px', color: '#64748b', fontSize: '0.85rem', fontWeight: 500 }}
                    onClick={() => setEmailOpen(o => !o)}>
                    <Mail size={16} />
                    Sign in with email
                    {emailOpen ? <ChevronUp size={14} style={{ marginLeft: 'auto' }} /> : <ChevronDown size={14} style={{ marginLeft: 'auto' }} />}
                </button>
            )}
                    ...
                    {!otpSent ? (
                        <>
                            <input type="email" value={email}
                                ...
                                autoFocus />
                            <button className="btn btn-secondary btn-sm" disabled={!!busy || !email.trim()}
                        ...
                        <>
                            ...
                            <input type="text" value={otp} maxLength={6}
                                ...
                                placeholder="123456"
                                style={{ padding: '9px 12px', borderRadius: 7, border: '1px solid #e2e8f0', fontSize: '1.1rem', letterSpacing: '0.25em', textAlign: 'center', fontFamily: 'monospace' }}
                                autoFocus />
                            <div style={{ display: 'flex', gap: 8 }}>
                                <button className="btn btn-ghost btn-sm" onClick={() => { setOtpSent(false); setOtp(''); }}>Back</button>
                                <button className="btn btn-primary btn-sm" disabled={!!busy || otp.length < 6}
