...
export default function EssayAssignmentModal({
    rubricId, rubricName, studentId, studentName,
    onClose, onOpenSlipSheet, onSaveAssignment, classStudents,
}: Props) {
    ...
    const handleSaveToDb = useCallback(async () => {
        ...
        else { setSaveError(result.error ?? 'Failed to save assignment'); }
        ...
        <Modal titleId="essay-assignment-title" onClose={onClose} maxWidth={560}>
            ...
            <div style={{ padding: 20, display: 'flex', flexDirection: 'column', gap: 14, maxHeight: '70vh', overflowY: 'auto' }}>
                ...
                {dbStatus.isConnected && (
                    <div style={{ background: 'color-mix(in srgb, var(--accent) 8%, transparent)', border: '1px solid color-mix(in srgb, var(--accent) 30%, transparent)', borderRadius: 8, padding: '10px 14px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                        <label style={{ display: 'flex', alignItems: 'center', gap: 8, cursor: 'pointer', fontSize: '0.875rem', fontWeight: 600 }}>
                            <input type="checkbox" checked={embedDb} onChange={e => setEmbedDb(e.target.checked)} style={{ accentColor: 'var(--accent)' }} />
                            <Database size={13} style={{ color: 'var(--accent)' }} />
                            Enable direct submission to database
                        </label>
                        {embedDb && (
                            <p style={{ margin: 0, fontSize: '0.8rem', color: 'var(--text-muted)', lineHeight: 1.5 }}>
                                Students will verify their email via OTP, then submit directly to your Supabase project. No copy-paste code needed. The backup code is still shown as a receipt.
                            </p>
                        )}
                    </div>
                )}
                ...
                {dbStatus.isConnected && embedDb && onSaveAssignment && (
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <button className="btn btn-secondary btn-sm" onClick={handleSaveToDb} disabled={saving || saved}>
                            {saved ? <><Check size={13} /> Saved to database</> : saving ? 'Saving…' : <><Database size={13} /> Save assignment to database</>}
                        </button>
                        {saveError && (
                            <span style={{ display: 'flex', alignItems: 'center', gap: 4, color: 'var(--red)', fontSize: '0.8rem' }}>
