...
- **Placement testing**: Author a routed/staged test (Test Builder → Mode: Placement) — tag each section with a target CEFR level and a pass-threshold routing rule deciding which section the student sees next based on their score in the current one. Students take one section at a time; results show the exact section-by-section path and per-section scores. The outcome is a provisional CEFR estimate — clearly badged as such — that feeds the student's CEFR overview and learning path until a real graded assessment replaces it.
