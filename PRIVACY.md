...
| Scanned handwriting images | _(not stored)_          | Photos/scans of handwritten work are OCR'd in memory and discarded; only the recognised text is kept, in the grade's feedback (see below) | **Yes** (transiently)    |
...
**Handwriting scans note:** The scan-and-OCR feature (a teacher-only grading tool — students never upload scans) recognises text from a photo or scanned PDF entirely on the teacher's device using Tesseract.js; the image itself is **not uploaded** and, in this release, is **not stored** — it is processed in memory and discarded once recognition completes. Only the recognised, teacher-corrected text is kept, inside the grade's feedback. (The backend includes storage/retention scaffolding — a private, owner-only `scans` bucket and a one-academic-year purge — for a future opt-in cloud-archival step; it is not enabled by the app yet, so no scan images are persisted.)
